...
def Omega1(T, lambda3, n, compare_both=True):
    ...
    t0 = 1 / np.pi
    s1 = scipy.integrate.quad(quad1, 0, t0, epsabs=1e-8, epsrel=1e-8, limit=50)[0]
    s2 = scipy.integrate.quad(quad2, t0, np.inf, epsabs=1e-8, epsrel=1e-8, limit=50)[0]
    ret = s1 * 2 + s2 / np.pi
...
def Omega1_1st_order_upper_bound(T, lambda3, n):
    return (
        1.2533 / T
        + 0.3334 * abs(lambda3) / T / n**0.5
        + 14.1961 / T**4
        + abs(lambda3) * np.exp(-(T**2) / 39.47842) / 3 / np.pi / n**0.5
    )
...
def I31(T, lambda3, K4, n, eps):
    ...
    return (
        ...
        + 0.036278 * e1(eps) * lambda3**2 / n
        + 0.32636
        * scipy.integrate.quad(quad, l, m, epsabs=1e-8, epsrel=1e-8, limit=50)[0]
    )
...
def I32(T, K3, K3_, K4, n, eps):
    return (
        ...
        / 3
        / n**0.5
        * J2(3, (2 * eps) ** 0.5 * (n / K4) ** 0.25, T / np.pi, K3_, K4, T, n)
    )
...
def I33(T, lambda3, K4, n, eps):
    return (
        ...
        / 3
        / n**0.5
        * J1(3, (2 * eps) ** 0.5 * (n / K4) ** 0.25, T / np.pi, T)
    )


def R1n(t, eps, lambda3, K4, n):
    temp = 1 / (1 - 3 * eps) ** 2
    return (U11n(t, K4 / n) + U12n(t, K4 / n)) / 2 * temp + e1(eps) * (
        t**8 * K4**2 / 2 / n**2 * (1 / 24 + P1n(eps) / 2 * temp) ** 2
...
def P1n(eps):
    return (
        ...
        + 48 * eps
        + 4 * eps**2
        + (96 * (2 * eps) ** 0.5 + 32 * eps + 16 * 2**0.5 * eps**1.5)
    ) / 576
...
def J2(p, l, m, K3_, K4, T, n):
    quad = (
        ...
        * u**p
        * np.exp(
            -(u**2) / 2 * (1 - 0.198324 * abs(u) * K3_ / n**0.5 - (K4 / n) ** 0.5)
        )
    )
    return (
        1 / T * scipy.integrate.quad(quad, l, m, epsabs=1e-8, epsrel=1e-8, limit=50)[0]
    )
...
def EX(p, mu):
    ...
    return scipy.integrate.quad(quad, -20, 20, epsabs=1e-8, epsrel=1e-8, limit=50)[0]
...
def error_difference(p, mu, a):
    ...
    return scipy.integrate.quad(quad, a, 20, epsabs=1e-8, epsrel=1e-8, limit=50)[0]
