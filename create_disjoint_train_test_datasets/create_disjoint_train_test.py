...
class DisjointTest(Dataset):
    def __init__(self, args, root, filename):
        self.args = args

        speaker_id_list = []

        df_test = pd.DataFrame(columns=["speaker_id", "wave"])
        ...
        for path, _, files in os.walk(root):
            for name in files:
                ...
                if name.endswith(".flac") and (len(spk_ID) <= self.args.n_speakers):
                    name_a = name.split(".")
                    name_b = name_a[0].split("-")
                    ...
                    if (
                        ...
                        df_test.loc[i] = [speaker_id] + [wave]
                        i += 1

        labels_test = pd.DataFrame(columns=["speaker_id", "gender"])
        ...
        dataset_test = pd.merge(
            df_test, labels_test, on="speaker_id"
        )  # merging the two dataframes on 'speaker_id' for testing.

        self.samples_test = dataset_test

        self.gender_test_list = sorted(set(dataset_test["gender"]))
        self.speaker_test_list = sorted(set(self.samples_test["speaker_id"]))

    def __getitem__(self, i):
        sample_test = self.samples_test

        gdr_test = sample_test["gender"][i]
        wave_test = sample_test["wave"][i]
        spk_test = sample_test["speaker_id"][i]

        feature_test = extract_features(wave_test, self.args).swapaxes(0, 1)

        # zero mean and unit variance
        feature_test = (feature_test - feature_test.mean()) / feature_test.std()

        return spk_test, gdr_test, feature_test
...
def preprocess(args):
    ...
    output_dir_path_test = Path(f"{output_dir_test}_{agt_i}")
    # output_dir_path_test = Path(output_dir_test)
    output_dir_path_test.mkdir(parents=True, exist_ok=True)
    ...
    infos_gender_speaker_test = {
        "n_mels": args.feature_dim,
        "speaker_gender": {
            speaker_name_test: []
            for speaker_name_test in dataset_test.speaker_test_list
        },
    }
    ...
    for speaker_name_test, gender_name_test, mel_tensor_test in tqdm(dataloader_test):
        speaker_name_test = speaker_name_test[0]
        gender_name_test = gender_name_test[0]
        mel_tensor_test = mel_tensor_test.squeeze(0)

        random_file_path_test = output_dir_path_test / f"uttr-{uuid4().hex}.pt"
        torch.save(mel_tensor_test, random_file_path_test)

        infos_gender_speaker_test["speaker_gender"][speaker_name_test].append(
            {
                "feature_path": random_file_path_test.name,
                "mel_len": mel_tensor_test.shape[0],
                "gender": gender_name_test,
            }
        )

        with open(output_dir_path_test / "metadata.json", "w") as f_test:
            json.dump(infos_gender_speaker_test, f_test, indent=2)
