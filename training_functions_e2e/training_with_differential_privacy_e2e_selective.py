...
def train_with_dp_e2e_selective(
    args,
    hparams,
    device,
    sub_sample,
    ...
    """Differantially private training excluding a given type of layer.
    ...
    if status_dp_dvec_cls == "adv":
        optimizer = opt["opt"](
            dvec_cls_e2e.parameters(),
            lr=hparams.lr_cls,
            amsgrad=True,
        )
    elif status_dp_dvec_cls == "dp":
        # Freeze the LSTM layer trained by public data and corresponding adversarial samples
        for name_q, q in dvec_cls_e2e.named_parameters():
            if model_collection["excluded_layer"] in name_q:
                q.requires_grad = False
            else:
                q.requires_grad = True

        optimizer = opt["opt"](
            [q for q in dvec_cls_e2e.parameters() if q.requires_grad],
            lr=hparams.lr_cls,
            amsgrad=True,
        )
    else:
