...
def train_with_dp_e2e_full(
    ...
    """Differantially private training.
    ...
    Args:
        ...
        status_dp_dvec_cls: The status for the dp classifier.
    ...
    if ckpt_dvec_cls is not None:
        ...
        if status_dp_dvec_cls == "dp":
            ...
            model_dict = dvec_cls_e2e.state_dict()
            model_dict.update(pretrained_dict)
            dvec_cls_e2e.load_state_dict(model_dict)
    ...
    for epoch in range(start_epoch, start_epoch + privacy_props["num_iter"]):
        ...
        # x_sub_private_adv, _ = attk(x_private, spk_idx_private)

        # x_pvt_concat = torch.cat([x_private, x_sub_private_adv], dim=0).reshape(
        #     -1,
        #     args.seg_len,
        #     args.feature_dim,
        # )
        # x_pvt_comb = x_pvt_concat[indx_comp]
        x_pvt_comb = x_private[indx_comp]

        # spk_pvt_concat = torch.cat([spk_idx_private, spk_idx_private]).view(-1)
        # spk_pvt_com = spk_pvt_concat[indx_comp]
        ...
        # epsilon = privacy_props["privacy_engine"].get_epsilon(hparams.delta)
        # epsilon = privacy_props["compute_eps_gdp"](
        #     epoch * hparams.points[0] / hparams.num_iter,
        #     hparams.sigma,
        #     len(privacy_props["dataset"]),
        #     privacy_props["bs"],
        #     hparams.delta,
        # )
